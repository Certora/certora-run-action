...
: "${CERTORA_SERVER:?Missing CERTORA_SERVER}"
...
# Select API domain
case "$CERTORA_SERVER" in
  production)
    DATA_API="data-api"
    ;;
  staging)
    DATA_API="data-api-stg"
    ;;
  vaas-dev)
    DATA_API="data-api-dev"
    ;;
  *)
    echo "::error title=Invalid Certora Server::Certora Server must be set to 'production', 'staging', or 'vaas-dev'."
    exit 1
    ;;
esac
